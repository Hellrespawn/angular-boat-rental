...
export class BoatController {
  ...
  private checkIfBoatInputIsCorrect(
    name: string,
    price: number,
    skipperNeeded: boolean,
    length: number,
    maxSpeed: number,
    sailOrMotor: 'sail' | 'motor'
  ): boolean {
    ...
    typeof name == 'string' &&
    typeof price == 'number' &&
    typeof skipperNeeded == 'boolean' &&
    typeof length == 'number' &&
    typeof maxSpeed == 'number' &&
    (sailOrMotor == 'sail' || sailOrMotor == 'motor')
      ? (fieldsAreCorrect = true)
  ...
  public async addBoat(
    ...
    if (
      this.checkIfBoatInputIsCorrect(
        name,
        price,
        skipperNeeded,
        length,
